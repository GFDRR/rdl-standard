...
def test_empty():
    ...
    empty_files_paths = [path for path in get_empty_files(excluded=('.git', '.ve', '_static', 'build', 'fixtures', 'flatten-tool', 'docson')) if "src/" not in path[0]]
...
def test_indent():
    ...
    """
    misindented_files_paths = [path for path in get_misindented_files(excluded=('.git', '.ve', '_static', 'build', 'fixtures', 'flatten-tool', 'docson')) if "src/" not in path[0]]
    warn_and_assert(
...
def test_invalid_json():
    ...
    warn_and_assert(
        get_invalid_json_files(excluded=('.git', '.ve', '_static', 'build', 'fixtures', 'flatten-tool', 'docson')), "{0} is not valid JSON: {1}", "JSON files are invalid. See warnings below."
    )
